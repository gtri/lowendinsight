...
defmodule GitModuleTest do
  ...
  setup_all do
    ...
    {:ok, gitlab_repo} = GitModule.clone_repo "https://gitlab.com/gitlab-com/gl-infra/infrastructure.git"
    [
      repo: repo,
      tag_repo: tag_repo,
  ...
  test "get contributor counts", context do
    ...
    {:ok, t_contributor_distribution, t_total} = GitModule.get_contributor_distribution(context[:tag_repo])
    assert Map.values(t_contributor_distribution) == [1, 24, 1, 4, 2, 1, 2, 1, 234, 5, 1, 200, 1, 1, 9, 19, 1, 1, 1, 1,
    1, 1, 3, 4, 3, 2]
    assert t_total == 524

    {:ok, bb_contributor_distribution, bb_total} = GitModule.get_contributor_distribution(context[:bitbucket_repo])
    assert Map.values(bb_contributor_distribution) == [30, 2]
    assert bb_total == 32

    {:ok, gl_contributor_distribution, gl_total} = GitModule.get_contributor_distribution(context[:gitlab_repo])
    assert Map.values(gl_contributor_distribution) == [1, 2, 10, 1, 2, 2, 22, 22, 5, 1, 1, 6, 3, 9, 3, 2, 1, 1, 1, 35, 29,
    1, 1, 6, 1, 1, 14, 4, 10, 1, 5, 1, 2, 3, 2, 6, 8, 1, 17, 5, 1, 11,
    1, 2, 3, 1, 6]
    assert gl_total == 273
  end
